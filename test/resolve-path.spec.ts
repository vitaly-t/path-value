...
describe('one property', () => {
    ...
    describe('for primitive types', () => {
        it('must call functions with context', () => {
            ...
            expect(resolve('test', 'length')).to.eql({chain: ['length'], idx: 0, value: 4});
        });
    });
    describe('for missing properties', () => {
        it('must resolve with undefined', () => {
            expect(resolve({}, 'one')).to.eql({chain: ['one'], idx: 0, value: undefined});
        });
    });
    describe('for functions', () => {
        it('must invoke correctly', () => {
            const obj1 = {one: () => 123};
            expect(resolve(obj1, 'one')).to.eql({chain: ['one'], idx: 0, value: 123});
            const obj2 = {
                one: 123, getValue() {
                    return this.one;
                }
            };
            expect(resolve(obj2, 'getValue')).to.eql({chain: ['getValue'], idx: 0, value: 123});
        });
    });
});
...
describe('multiple properties', () => {
    describe('for valid simple names', () => {
        it('must resolve', () => {
            ...
            expect(resolve(obj1, 'one.two')).to.eql({chain: ['one', 'two'], idx: 1, value: 12});
    ...
    describe('for invalid simple names', () => {
        it('must resolve', () => {
            const obj1 = {};
            expect(resolve(obj1, 'one.two')).to.eql({chain: ['one', 'two'], idx: 0, errorCode: ParseErrorCode.stopped});
            const obj2 = {one: {}};
